...
export default class TriggerActionForm extends LightningElement {
  ...
  async connectedCallback() {
    ...
    if (this.isCreating) {
      ...
      this.selectedContext__c =
        Object.keys(this.implementedContexts).find(
          (key) => this.implementedContexts[key]
        ) || "";
    }
  }
  ...
  handleInputChange(event) {
    ...
    const value =
      event.detail !== undefined ? event.detail.value : event.target.value;
  ...
  handleNumberChange(event) {
    ...
    this.formData = {
      ...this.formData,
      [field]: event.target.value ? parseInt(event.target.value, 10) : null
    };
  }
  ...
  async handleApexClassChange(event) {
    ...
    if (className) {
      try {
        ...
        this.showError(
          "Error",
          "Failed to detect contexts: " + error.body?.message || error.message
        );
      } finally {
