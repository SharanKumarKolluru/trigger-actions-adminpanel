...
import getSessionId from "@salesforce/apex/FlowVisualizerController.getSessionId";
import getOrgDomainUrl from "@salesforce/apex/FlowVisualizerController.getOrgDomainUrl";
...
const DIAGRAM_STYLES = `
  ...
  .diagram-canvas svg .state label,
  .diagram-canvas svg .state text {
...
export default class FlowVisualizer extends LightningElement {
  ...
  renderDiagram() {
    try {
      ...
      // Render to SVG
      window.mermaid.render(chartId, this.mermaidCode, (svgCode) => {
        this.isReady = true;
